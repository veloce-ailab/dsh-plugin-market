...
function isMarketPackageName(value) {
...
async function npmPluginNames() {
    ...
    for (;;) {
        ...
        if (typeof last !== 'string' || last <= start)
            return names;
        start = `${last}\u0000`;
    }
}
...
async function installedVersion(root, name) {
    try {
...
async function npmPackages(root) {
    if (packageCache !== undefined && packageCache.expiresAt > Date.now())
...
async function installPackage(root, request) {
...
function installRequest(value) {
    ...
    if (source === undefined || !isMarketPackageName(source.name))
        throw new Error('request.name must be a DSH plugin package');
...
function addPluginRequest(value) {
    const source = record(value);
    if (source === undefined || !isMarketPackageName(source.name))
        throw new Error('request.name must be a DSH plugin package');
...
async function appendPluginPatch(path, existing, request) {
        ...
        handler: async (request, response) => {
            try {
                ...
                if (request.method === 'GET') {
                    ...
                    if (name !== null) {
                        if (!isMarketPackageName(name))
                            throw new Error('package is not in the DSH plugin namespace');
                ...
                if (!metadata.versions.includes(requestData.version))
                    throw new Error('selected package version was not found on npm');
